...
/**
 * Generic CareStack raw-payload modal — reused for both Patient and
 * Appointment full-record views. Filter by field name or value, esc /
 * backdrop closes.
 */
export function CsRawDialog({
  title,
  id,
  onClose,
  data,
  ...
  const totalFields = data ? Object.keys(data).length : 0;
  const populatedFields = data
    ? Object.entries(data).filter(([, v]) => v !== null && v !== "").length
    : 0;
